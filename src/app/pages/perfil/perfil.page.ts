import { Component, OnInit, ViewChild } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormBuilder, FormControl, FormGroup, FormsModule, ReactiveFormsModule, Validators } from '@angular/forms';
import { IonicModule, ModalController, NavController, ToastController } from '@ionic/angular';
import { Photo } from '@capacitor/camera';
import { DataService } from 'src/app/services/data.service';
import { FirebaseService } from 'src/app/services/firebase.service';
import { PhotoService } from 'src/app/services/photo.service';
import { getAuth, reauthenticateWithCredential, signInWithEmailAndPassword, updatePassword } from 'firebase/auth';
import { sign } from 'crypto';

@Component({
  selector: 'app-perfil',
  templateUrl: './perfil.page.html',
  styleUrls: ['./perfil.page.scss'],
  standalone: true,
  imports: [IonicModule, CommonModule, FormsModule, ReactiveFormsModule]
})
export class PerfilPage implements OnInit {
  credenciales!: any;
  alumnoActual: any;
  fechaSeleccionada!: string;
  alumnoAvatar: string = '';
  auth = getAuth();
  user = this.auth.currentUser;
  mostrar: boolean = false;
  prueba!: string;
  idFoto: string;
  avatarPerfil!: Photo;
  guardarFoto: boolean = false;
  foto: string = 'Elige una foto de perfil!';
  avatar?: string = 'data:image/png;base64,';
  formReg: FormGroup = this.fb.group({
    email: new FormControl({ value: '', disabled: true }, [Validators.required, Validators.email]),
    password: new FormControl({ value: '', disabled: true }, [Validators.required, Validators.minLength(6)]),
    nombre: new FormControl('', Validators.required),
    apellidos: new FormControl('', Validators.required),
    cinturon: new FormControl('blanco'),
    peso: new FormControl(),
    telefono: new FormControl('', [Validators.required, Validators.minLength(9)]),
    tarifa: new FormControl('iniciacion', Validators.required),
    fotoPerfil: new FormControl('', Validators.required),
  })
  formContra: FormGroup = this.fb.group({
    actual: new FormControl('', [Validators.required, Validators.minLength(6)]),
    nueva: new FormControl('', [Validators.required, Validators.minLength(6)]),
    confirmacion: new FormControl('', [Validators.required, Validators.minLength(6)]),
  })

  constructor(private fb: FormBuilder,
    private firebase: FirebaseService,
    public photoService: PhotoService,
    private navCtrl: NavController,
    private toastController: ToastController,
    private data: DataService,
    private modalController: ModalController) {
    this.idFoto = new Date().getTime() + '.jpeg';
    if (this.user) {
      this.firebase.recuperarAlumno(this.user.email!).then(
        (alumno) => {
          this.alumnoActual = alumno
          this.photoService.descargarAvatar(alumno.avatar).then(
            (resp) => {
              this.avatar = resp
            })
          console.log(this.alumnoActual)
          this.rellenarCampos()
        }
      )
    }
  }

  addNewFromGallery() {

    this.photoService.addNewFromGallery(this.guardarFoto, this.idFoto)
      .then((resp) => {
        this.avatar = resp.webviewPath;
        this.foto = resp.filepath;
        this.formReg.controls['fotoPerfil'].setValue(resp.filepath)


      });
  }
  ngOnInit() { }
  async onSubmit() {
    if (this.formReg.controls['fotoPerfil'].touched) {
      this.mostrar = true;
      this.guardarFoto = true;
      this.photoService.savePicture(this.guardarFoto, this.photoService.photo, this.idFoto);
    }else{
        this.foto=this.avatar!;
    }

    await this.firebase.actualizarAlumno(
      this.alumnoActual.correo,
      this.formReg.value.nombre,
      this.formReg.value.apellidos,
      this.formReg.value.telefono,
      this.formReg.value.cinturon,
      this.formReg.value.peso,
      this.formReg.value.tarifa,
      this.foto

    ).then(
      (resp) => {
        this.presentToast('Datos modificados correctamente ☺');
        if (this.user) {
          this.firebase.recuperarAlumno(this.user.email!).then(
            (alumno) => {
              this.alumnoActual = alumno
              this.photoService.descargarAvatar(alumno.avatar).then(
                (resp) => {
                  this.avatar = resp
                })
              this.formReg.reset();
              this.rellenarCampos();
              console.log(this.alumnoActual)
            }
          )
        }

      }
    ).catch(
      (resp) => {
        this.presentToast('No se ha podido modificar los datos, vuelva a intentarlo más tarde');
      }
    );

  }
  volverInicio() {
    this.navCtrl.navigateForward('/folder');

  }
  campoEsValido(campo: string) {
    return (
      this.formReg.controls[campo].errors &&
      this.formReg.controls[campo].touched
    );
  }
  rellenarCampos() {
    console.log(this.alumnoActual)
    this.formReg.controls['email'].setValue(this.alumnoActual.correo);
    this.formReg.controls['password'].setValue(this.alumnoActual.password);
    this.formReg.controls['nombre'].setValue(this.alumnoActual.nombre);
    this.formReg.controls['apellidos'].setValue(this.alumnoActual.apellidos);
    this.formReg.controls['cinturon'].setValue(this.alumnoActual.cinturon);
    this.formReg.controls['peso'].setValue(this.alumnoActual.peso);
    this.formReg.controls['tarifa'].setValue(this.alumnoActual.tarifa);
    this.formReg.controls['fotoPerfil'].setValue(this.alumnoActual.avatar);
    this.formReg.controls['telefono'].setValue(this.alumnoActual.telefono);


  }
  cambiaContra() {

    // const auth = getAuth();
    // const user = auth.currentUser;
    // signInWithEmailAndPassword(auth, this.alumnoActual.correo, this.alumnoActual.password)
    //   .then((userCredential) => {
    //     this.credenciales = userCredential;
    //     // Signed in
    //     const user = userCredential.user;
    //     // ...
    //   })
    //   .catch((error) => {
    //     const errorCode = error.code;
    //     const errorMessage = error.message;
    //     console.log(error)
    //   });

    // const credential = this.credenciales;

    // reauthenticateWithCredential(user!, credential).then(() => {


    //   // User re-authenticated.
    // }).catch((error) => {
    //   // An error ocurred
    //   // ...
    // });

    const nuevaContra = this.formContra.value.confirmacion;
    updatePassword(this.user!, nuevaContra).then(() => {
      this.firebase.actualizarContra(this.alumnoActual.correo, nuevaContra).then(
        () => {
          this.formContra.reset();
          this.presentToast('Se ha modificado su contraseña')

        }
      )
      // Update successful.
    }).catch((error) => {
      // An error ocurred
      // ...
      this.presentToast('Ha ocurrido un error')

    });


  }

  nuevaContraValida(): boolean {
    let valida: boolean = false;
    if ((this.formContra.value.actual === this.alumnoActual.password)
      &&
      (this.formContra.value.nueva === this.formContra.value.confirmacion && this.formContra.valid)) {
      valida = true;

    }
    return valida;
  }
  async presentToast(mensaje: any) {
    const toast = await this.toastController.create({
      message: mensaje,
      duration: 4000 // duración en milisegundos
    });
    toast.present();
  }

  promptForCredentials() {


    signInWithEmailAndPassword(this.auth, this.alumnoActual.correo, this.alumnoActual.password)
      .then((userCredential) => {
        this.credenciales = userCredential;
        // Signed in
        const user = userCredential.user;
        // ...
      })
      .catch((error) => {
        const errorCode = error.code;
        const errorMessage = error.message;
        console.log(error)
      });



  }

}


